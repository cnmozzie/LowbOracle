...
(async () => {
  ...
  let depositId = await maticOracleContract.methods.depositId().call()
  let id = Number(depositId) + 1
  console.log('start listening from ...', id)
  setInterval( async () => {
    let block = await maticWeb3.eth.getBlockNumber()
    let transactions = await maticOracleContract.getPastEvents('DepositForEvent', { filter: {id: id}, fromBlock: block-500, toBlock: 'latest' })
    if (transactions.length > 0) {
      let value = transactions[0].returnValues
      console.log('* New DepositForEvent event. id: ', value.id)
      id++;
      await bscOracleContract.methods.withdrawTo(value.amount, value.to, value.id).send({ from: ownerAddress, gas: 150000, gasPrice: 10000000000 })
