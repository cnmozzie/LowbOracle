...
(async () => {
  ...
  setInterval( async () => {
	let transaction = await oecOracleContract.methods.transactions(id).call()
    if (transaction.amount > 0) {
      ...
      if (transactions.length > 0) {
        ...
        id++;
        await oecOracleContract.methods.withdrawTo(value.amount, value.to, value.id).send({ from: ownerAddress, gas: 200000, gasPrice: 10000000000 })
